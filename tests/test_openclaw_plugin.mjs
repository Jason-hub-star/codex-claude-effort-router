...
const router = join(here, "..", "router", "model_orchestrator.py");
...
// manifest shape
assert.equal(plugin.id, "model-orchestrator");
...
// advisory by default: context prepended, no model override
{
  ...
  assert.match(build.prependContext, /\[MODEL ORCHESTRATOR\] lane=CRITICAL/);
...
// missing router: fail open on both hooks
{
  ...
  const saved = { router: process.env.MODEL_ORCHESTRATOR_ROUTER, home: process.env.HOME };
  process.env.MODEL_ORCHESTRATOR_ROUTER = "/nonexistent/too.py";
  process.env.HOME = mkdtempSync(join(tmpdir(), "model-orchestrator-"));  // no installed fallback either
  plugin.register(api);
  process.env.MODEL_ORCHESTRATOR_ROUTER = saved.router;
