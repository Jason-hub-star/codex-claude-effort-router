...
- Installer: OpenCode/OpenClaw selections now require Node 22 or newer before any files are written, with missing-Node and Node-18 regressions. An upgrade also removes the exact pre-rename Codex/Claude hook entry before adding the Model Orchestrator handler, preventing duplicate routing context.
