...
- 2026-09-08: resumed result `bench/results/exp2-fastlane-20260908.jsonl` completed 75/75, with no timeout, fatal provider error, or failed verification. `enforce-low` mean reasoning was 30.3 versus `enforce=medium` 60.3 (-49.8%); both passed 15/15. Sealed Case A applies, so the fast per-prompt effort default changed to `low`. Cases D and E: advisory was only 15% above none, while always-low beat none. Other lane defaults and Codex/Claude medium profile targets remain unchanged.
